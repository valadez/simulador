...
FACT* CreateFact(MEANING subject, MEANING verb, MEANING object, unsigned int properties)
{
	...
	if (trace & TRACE_INFER && currentFact)  Log(STDUSERLOG," Found %d ", Fact2Index(currentFact));
	if (currentFact) return currentFact;
	currentFact = CreateFastFact(subject,verb,object,properties);
	if (trace & TRACE_INFER && currentFact)  Log(STDUSERLOG," Created %d ", Fact2Index(currentFact));
...
char* EatFact(char* ptr,unsigned int flags,bool attribute)
{
	...
	{
		if (flags & FACTSUBJECT) F = GetSubjectHead(Index2Fact(subject));
		else F = GetSubjectHead(subject);
		while (F)
		{
