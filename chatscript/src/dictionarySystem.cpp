...
static char* predefinedSets[] = //  some internally mapped concepts not including emotions from LIVEDATA/interjections
{
	...
	"~pos","~sys","~role","~daynumber","~yearnumber","~dateinfo",
    NULL
};
...
static int call = 0;
char* AllocateString(char* word,size_t len,int bytes,bool clear)
...
WORDP StoreWord(char* word, uint64 properties)
{
	...
	if (!server) // servers dont want long lists of bugs from strange inputs
	{
		if (!*word) //   we require something 
		{
			...
			char* xxvalue = GetUserVariable("$input"); // in case we want to debug using the sentence
...
void WordnetLockDictionary() // dictionary before build0 layer 
{
	...
	FILE* out = FopenBinaryWrite("TMP/prebuild0.bin","wb"); // binary file, no BOM
...
#ifndef DISCARDSCRIPTCOMPILER
	FILE* out = FopenBinaryWrite("TMP/prebuild1.bin","wb"); // binary file, no BOM
	if (out)
	{
		char x = 0;
...
{
	FILE* out = FopenBinaryWrite(UseDictionaryFile("dict.bin"),"wb"); // binary file, no BOM
	if (!out) return;
...
void WriteDictionaryFlags(WORDP D, FILE* out)
{
	...
	while (properties)
	{
		if (properties & bit)
		{
			...
			else if (bit & PATTERN_WORD) label = NULL;			// internal marker or transient script compiler flag
...
{
    char word[MAX_WORD_SIZE];
    FILE* in = FopenReadOnly(name); // queries
    if (!in) return;
    while (ReadALine(readBuffer,in)  >= 0) 
    {
        if (*readBuffer == '#' ||  *readBuffer == 0) continue;
...
void DumpDictionaryEntry(char* word,unsigned int limit)
{
	...
	while (bit)
	{
		...
		{
			char word[MAX_WORD_SIZE];
			if (!(bit & ESSENTIAL_FLAGS)) Log(STDUSERLOG,"%s ",MakeLowerCopy(word,FindName2ByValue(bit)));
		}
		bit >>= 1;
	}
	...
	if (D->internalBits & TRACE_MACRO ) Log(STDUSERLOG,"being-traced ");
