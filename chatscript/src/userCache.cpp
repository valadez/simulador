...
int volleyLimit =  -1;					// default save user records to file every n volley (use default 1 if this value is unchanged by user)
...
static void WriteCache(unsigned int which,size_t size)
{
	...
	FILE* out = FopenBinaryWrite(ptr,"wb"); // binary file
...
char* GetFileRead(char* user,char* computer)
{
    ...
    FILE* in = (!buffer) ? NULL : FopenReadWritten(word); // user topic file
	...
	if (in) // read in data if file exists
	{
		...
		if ((int)actualSize != -1) 
		{
			...
			fseek(in, 0, SEEK_SET);
			size_t readit = fread(buffer,1,actualSize,in);	// read it all in, including BOM
			buffer[readit] = 0;
