...
static void* MainChatbotServer()
{
	...
	if (setjmp(scriptJump[SERVER_RECOVERY])) // crashes come back to here
	{
		...
		if (*bad) strcpy(mainOutputBuffer,bad);
		else strcpy(mainOutputBuffer,"Hey, sorry. I forgot what I was thinking about.");
 ...
 _try { // catch crashes in windows
	...
	while (1)
	{
		...
		if (test >= INPUT_BUFFER_SIZE) strcpy(mainInputBuffer,"too much data");
		else strcpy(mainInputBuffer,ptr); // xfer user message to our incoming feed
		echo = false;
		if (serverPreLog) Log(SERVERLOG,"ServerPre: %s %s %s\r\n",user,bot,mainInputBuffer);

		*((int*) clientBuffer) = PerformChat(user,bot,mainInputBuffer,ip,mainOutputBuffer);	// this takes however long it takes, exclusive control of chatbot.
		ServerTransferDataToClient();
	}
#ifdef WIN32
