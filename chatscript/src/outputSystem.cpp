...
char* ReadCommandArg(char* ptr, char* buffer,FunctionResult& result,unsigned int control)
{
...
char* ReadShortCommandArg(char* ptr, char* buffer,FunctionResult& result,unsigned int control)
{
	if (control == 0) control |= OUTPUT_KEEPSET | OUTPUT_NOTREALBUFFER | OUTPUT_ONCE | OUTPUT_NOCOMMANUMBER;
	else control |= OUTPUT_ONCE | OUTPUT_NOCOMMANUMBER;
...
void ReformatString(char* input,char* output, FunctionResult& result,unsigned int controls) // take ^"xxx" format string and perform substitutions on variables within it
{
	...
	*output = 0;
	...
	while (input && *input)
	{
		if (*input == '$' && (IsAlphaUTF8(input[1]) ||  input[1] == '$')) // user variable
		{
			input = ReadCompiledWord(input,var,false,true);
			char* value = GetUserVariable(var);
		...
		{
			char* base = input++;
			if (IsDigit(*++input)) ++input; // after 2nd digit
...
static char* Output_Backslash(char* word, char* ptr, bool space,char* buffer, unsigned int controls,FunctionResult& result)
{
	...
	if (word[1] == 'r' && !word[2]) 
	{
		return ptr;		// ignore \r (built into \n)
	}
	if (word[1] == 'n')  //   \n
	{
		CONDITIONAL_SPACE();
...
static char* Output_Function(char* word, char* ptr,  bool space,char* buffer, unsigned int controls,FunctionResult& result,bool once)
{
	...
	{
		CONDITIONAL_SPACE();
		ReformatString(word+2,buffer,result);
	}
	...
	else // functions or ordinary words
	{
		...
		{
			if (*currentRuleOutputBase && (!strcmp(word,"^gambit") || !strcmp(word,"^respond") || !strcmp(word,"^reuse") || !strcmp(word,"^retry")  )) // leaving current rule
			{
...
retry:
		...
		case '(': case '{':
 		...
 		case '[':  //   random choice. process the choice area, then resume in this loop after the choice is made
			ptr = Output_Bracket(word, ptr, space, buffer, controls,result);
			break;
		...
		case ':': // a debug command?
		{
			...
			if (D && D->x.debugIndex) 
			{
				...
				ptr = NULL;
				if (quitting == true) 
					myexit("quit requested from script");
		...
		if (*word == ENDUNIT) 
		{
			...
			else if (once && word[1] != '^'){;}	// have our answer (unless it was a function variable substitution)
			else
			{
				strcpy(word,buffer);
				*buffer = 0;
