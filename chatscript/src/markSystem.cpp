	...
	for (int i = startSentence; i < (int)endSentence; ++i)
	{
		...
		*rawbuffer = 0;
		...
		while ((++k + i) <= endSentence)
		{
			...
			if (!IsAlphaUTF8OrDigit(*wordStarts[i+k]) && *wordStarts[i+k] != '\'' ) break; // we only composite words, not punctuation except possessives or quoted stuff except we can include them as part of something
			NextInferMark();
...
void MarkAllImpliedWords()
{
    ...
    for (i = startSentence; i <= endSentence; ++i) //   mark that we have found this word, either in original or canonical form
    {
		...
		pos->inferMark = inferMark; // dont mark these supersets
		sys->inferMark = inferMark; // dont mark these supersets
		role->inferMark = inferMark; // dont mark these supersets
	...
	//		if (originalLower[i]) flags |= originalLower[i]->properties & (NOUN_SINGULAR|NOUN_PLURAL|NOUN_PROPER_SINGULAR|NOUN_PROPER_PLURAL);
		...
		if (flags & (ADJECTIVE_NUMBER|NOUN_NUMBER) && IsPlaceNumber(original)) flags |= PLACE_NUMBER;
		if (!stricmp(wordCanonical[i],"be"))
		{
			if (!stricmp(original,"am") || !stricmp(original,"was")) flags |= FIRST_PERSON;
		}
		if (flags & NOUN_INFINITIVE) // transcribe back to verb only, leaving noun_infinitive status and not verb tense status
		{
		...
		if (wordStarts[i][1] && (wordStarts[i][1] == ':' || wordStarts[i][2] == ':')) // time info 1:30 or 11:30
		{
			...
			{
				AddSystemFlag(originalLower[i],ACTUAL_TIME);
			}
		}
		...
		if (allOriginalWordBits[i] & ( NOUN_NUMBER | ADJECTIVE_NUMBER))   // a date can become an idiom, marking it as a proper noun and not a number
		{
		...
		if (finalPosValues[i] & ADJECTIVE_NOUN) StdMark(MakeTypedMeaning(OL,0,NOUN), i, i); //  mark word as a noun
		else StdMark(MakeTypedMeaning(OL,0,restriction), i, i);
		...
		{
			if (finalPosValues[i] & ADJECTIVE_NOUN) StdMark(MakeTypedMeaning(OU,0,NOUN), i, i); //  mark word as a noun first, adjective is not normal
			else StdMark(MakeTypedMeaning(OU,0,restriction), i, i);
       	}
		...
		{
			StdMark(MakeTypedMeaning(CL,0,VERB), i, i);
		}
		...
		{
			StdMark(MakeTypedMeaning(CL,0,NOUN), i, i);
		}
 		else StdMark(MakeTypedMeaning(CL,0, (unsigned int)(finalPosValues[i] & BASIC_POS)), i, i);

 		markLength = 0;
	    if (trace & TRACE_PREPARE || prepareMode == PREPARE_MODE) Log(STDUSERLOG," // "); //   close canonical form lowercase
 		
		// mark upper case canonical ONLY if existed (someone wants it) and lower doesnt exist
		if (IS_NEW_WORD(CU) && CL) {;}
		else StdMark(MakeTypedMeaning(CU,0, (unsigned int)(finalPosValues[i] & BASIC_POS)), i, i);

		// canonical word is a number (maybe we didn't register original right) eg. "how much is 24 and *seven"
		if (IsDigit(*wordCanonical[i]) && IsNumber(wordCanonical[i])) MarkFacts(Mnumber,i,i);  
