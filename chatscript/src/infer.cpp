...
static bool ConceptPropogateTest(MEANING M,unsigned int mark,unsigned int depth)  // is this meaning ultimately in a marked set
{
	...
	while (F)
	{
		if (F->verb == Mmember && F->subject == M)
		{
			...
			if (trace & TRACE_INFER)  Log(STDUSERLOG," %d->%s ",depth,D->word);
...
#		the value to process will be marked AND may or may not get stored, depending on following flag being q or m
	...
	if (!strncmp(control,"queue",5))  control = SkipWhitespace(control+5);  // just a label defining the field
	if (trace & TRACE_QUERY) 
	{
		char* colon = strchr(control+1,':');
		if (colon) *colon = 0;
		if (control[1]) Log(STDUSERLOG,"@@@ Control2 queue use: %s\r\n",control+1);
		if (colon) *colon = ':';
	}
	...
	if (*control) control = SkipWhitespace(control+1); // skip over the : and past any white space
	if (!strncmp(control,"match",5)) control = SkipWhitespace(control + 5);  // just a comment label defining what the field does
	if (trace & TRACE_QUERY && *control) 
	{
		char* colon = strchr(control+1,':');
		if (colon) *colon = 0;
		if (control[1]) Log(STDUSERLOG,"@@@ Control3 match requirements: %s\r\n",control+1); // if there is data
		if (colon) *colon = ':';
	}
	...
	if (*control && --control) while (*++control && *control != ':' )
	{
		...
		case '!': //   do NOT match this
			...
			if (*control == 'n')
			{
				if (trace & TRACE_QUERY) Log(STDUSERLOG," don't match concepts with noconceptlist marking ");
				noSystemFlag = true;
		...
		case 'm': 
			...
			if (trace & TRACE_QUERY) Log(STDUSERLOG," object must ultimately be member of set marked #%c ",*control);
			break;
		default: 
	...
	if (*control) control = SkipWhitespace(control+1); // skip over the : and past any white space
	if (!strncmp(control,"walk",4)) control = SkipWhitespace(control +4);
	if (trace & TRACE_QUERY && *control) 
	{
		char* colon = strchr(control+1,':');
		if (colon) *colon = 0;
		if (control[1]) Log(STDUSERLOG,"@@@ Control4 riccochet: %s\r\n",control+1);
		if (colon) *colon = ':';
	}
	if (*control && --control) while (*++control && *control != '|')
	{
	...
	while (scanIndex < queueIndex)
	{
		...
		if (baseFlags & FACTTYPE) // q data is facts
		{
			...
			else if (baseOffset == 2) F = GetObjectHead(F);
			index = 0;
		}
