...
bool serverPreLog = false;					// show what server got BEFORE it works on it
...
FILE* FopenBinaryWrite(const char* name, const char* how) // writeable file path
{
	char path[MAX_WORD_SIZE];
...
unsigned int Log(unsigned int channel,const char * fmt, ...)
{
    ...
    *at = 0;
    ...
    if (((echo||localecho) && channel == STDUSERLOG) || (channel == STDDEBUGLOG)  || (channel == SERVERLOG) ) fwrite(buffer,1,bufLen,stdout);
    FILE* out = NULL;
