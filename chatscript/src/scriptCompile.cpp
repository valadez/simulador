	...
	*word = 0;
	...
	if (*word == '#') // is this a constant from dictionary.h? or user constant
	{
		...
		else // system constant
		{
			...
			{
				if (IsAlphaUTF8(word[1])) BADSCRIPT("Bad numeric # constant %s",word)
				*ptr = 0;
				*word = 0;
			}
		}
	...
	{
		char* at = word;
		while (IsAlphaUTF8(*++at) );  // find end of word
...
{
	return (!stricmp(word,":quit")  || !stricmp(word,"replace:")  || !stricmp(word,"query:")  || !stricmp(word,"concept:") || !stricmp(word,"data:") || !stricmp(word,"plan:") 
		...
		!stricmp(word,"bot:") || !stricmp(word,"topic:") || (*word == ':' && IsAlphaUTF8(word[1])) );	// :xxx is a debug command
}
...
static void ValidateCallArgs(WORDP D,char* arg1, char* arg2,char argset[50][MAX_WORD_SIZE])
{
	...
	{
		if (stricmp(arg1,"conjugate") && stricmp(arg1,"raw") && stricmp(arg1,"syllable") && stricmp(arg1,"ADJECTIVE") && stricmp(arg1,"ADVERB")   && stricmp(arg1,"VERB") && stricmp(arg1,"AUX") && stricmp(arg1,"PRONOUN") && stricmp(arg1,"TYPE") && stricmp(arg1,"HEX32")&& stricmp(arg1,"HEX64")
 			...
 			BADSCRIPT("CALL- 12 1st argument to ^pos must be SYLLABLE or VERB or AUX or PRONOUN or NOUN or ADJECTIVE or ADVERB or DETERMINER or PLACE or CAPITALIZE or UPPERCASE or LOWERCASE or CANONICAL or INTEGER or HEX32 or HEX64 - %s",arg1)
	}
	...
	{
		if (stricmp(arg1,"RULE") && stricmp(arg1,"CALL") && stricmp(arg1,"TOPIC") && stricmp(arg1,"SENTENCE") && stricmp(arg1,"INPUT")  && stricmp(arg1,"PLAN")) 
	...
	{
		if (stricmp(arg1,"RULE") && stricmp(arg1,"CALL") && stricmp(arg1,"TOPIC") && stricmp(arg1,"SENTENCE") && stricmp(arg1,"INPUT")) 
	...
	else if (!stricmp(D->word,"^nofail"))
	{
		if (stricmp(arg1,"RULE") && stricmp(arg1,"TOPIC") && stricmp(arg1,"SENTENCE") && stricmp(arg1,"INPUT")) 
 			BADSCRIPT("CALL- 16 1st argument to ^nofail must be RULE or TOPIC or SENTENCE or INPUT - %s",arg1)
	}
	else if (!stricmp(D->word,"^compute") && IsAlphaUTF8(*arg2))
	{
		char* op = arg2;
		if (stricmp(op,"+") && stricmp(op,"plus") && stricmp(op,"add") && stricmp(op,"and") &&
			...
			stricmp(op,"root") && stricmp(op,"square_root")  && stricmp(op,"^") && stricmp(op,"^^") && stricmp(op,"power") && stricmp(op,"exponent")) 
...
{	//   returns with no space after it
	...
	*data++ = ' ';	
	...
	while (ALWAYS) //   read as many tokens as needed to complete the call, storing them locally
	{
		...
		if (oldContext && IsAlphaUTF8(*word) && stricmp(name,"^incontext") && stricmp(name,"^reuse") )  WritePatternWord(word);
	...
	*--mydata = 0;  //   remove closing paren
	...
	{
		if (stricmp(arg1,"topic") && stricmp(arg1,"rule") && stricmp(arg1,"rejoinder") && stricmp(arg1,"inputrejoinder") && stricmp(arg1,"outputrejoinder") ) BADSCRIPT("CALL-19 Disable 1st arg must be TOPIC or RULE or INPUTREJOINDER or OUTPUTREJOINDER - %s",arg1)
	...
	else if (info) // system function
	{
		if (argumentCount != info->argumentCount && info->argumentCount != VARIABLE_ARG_COUNT) 
			BADSCRIPT("CALL-60 Incorrect argument count to system function %s- %d instead of %d",name,argumentCount,info->argumentCount & 255)
	}
	...
	{
		if (D->w.fndefinition && argumentCount != MACRO_ARGUMENT_COUNT(D)) 
			BADSCRIPT("CALL-60 Incorrect argument count to macro %s- %d instead of %d",name,argumentCount,MACRO_ARGUMENT_COUNT(D))
	}
...
static void SpellCheckScriptWord(char* input,int startSeen,bool checkGrade) 
{
	...
	else if (!D || (!(D->properties & NORMAL_WORD) && !(D->systemFlags & PATTERN_WORD)))
	{
		...
		wordStarts[0] = wordStarts[1] = wordStarts[2] = wordStarts[3] = "";
		wordCount = 1;
	...
	while (ALWAYS) //   read as many tokens as needed to complete the definition
	{
		...
		if (comparison) //   is a comparison of some kind
		{
			...
			*comparison = c;
			...
			else if (IsAlphaUTF8DigitNumeric(*rhs)  )  WritePatternWord(rhs);		//   ordinary token
			else if (*rhs == '~') 
			{
				MakeLowerCase(rhs);
				CheckSetOrTopic(rhs);	
...
pattern: 
		...
		{
			if (*word != '$' && *word != '_' && *word != '@' && *word != '^'  && *word != '%' && *word != '?' ) BADSCRIPT("IF-10 existence test - %s. Must be ($var) or (%var) or (_#)  or (?) or (@#) or (~concept) ",word)
			strcpy(data,word);
			data += strlen(word);
		}
...
char* ReadOutput(char* ptr, FILE* in,char* &data,char* rejoinders,char* supplement,WORDP call)
{
	...
	*hold = 0;
	while (ALWAYS) //   read as many tokens as needed to complete the responder definition
	{
		...
		if (*word == '^' && word[1] != '=' && word[1] != '"' && word[1] != '\'' && word[1] != '$' && word[1] != '_' && word[1])
		{
...
static char* ReadMacro(char* ptr,FILE* in,char* kind,uint64 build)
{
	...
	*data = 0;
	...
	while (gettingArguments) //   read as many tokens as needed to get the name and argumentList
	{
		...
		if (parenLevel == 0 && !stricmp(word,"variable"))
		{
	...
	AddInternalFlag(D,FUNCTION_NAME|build|typeFlags);
...
static char* ReadKeyword(char* word,char* ptr,bool &notted, bool &quoted, MEANING concept,uint64 type,bool ignoreSpell,uint64 build)
{
	...
	*currentTopicName = 0;
	...
	while (ALWAYS) //   read as many tokens as needed to complete the definition
	{
		...
		if (!*currentTopicName) //   get the topic name
		{
			...
			if (topicName && topicName->internalBits & CONCEPT && !(topicName->internalBits & TOPIC) && topicName->internalBits & (BUILD0|BUILD1)) BADSCRIPT("TOPIC-1 Concept already defined with this topic name %s",currentTopicName)
			...
			AddInternalFlag(topicName,build|CONCEPT|TOPIC);
		...
		default:
			...
			else if (!keywordsDone) ptr = ReadKeyword(word,ptr,notted,quoted,topic,0,false,0);//   absorb keyword list
...
static char* ReadRename(char* ptr, FILE* in,uint64 build)
{
	...
	while (ALWAYS) //   read as many tokens as needed to complete the definition
	{
		...
		AddInternalFlag(D,RENAMED|build); 
...
static char* ReadPlan(char* ptr, FILE* in,uint64 build)
{
	...
	*planName = 0;
	...
	AddInternalFlag(D,FUNCTION_NAME|build|IS_PLAN_MACRO);
...
static char* ReadQuery(char* ptr, FILE* in, uint64 build) // readquery: name "xxxxx" text
{
	while (ALWAYS) //   read as many tokens as needed to complete the definition (must be within same file)
	{
		...
		WORDP D = StoreWord(word);
		AddInternalFlag(D, QUERY_KIND|build);
	...
	*conceptName = 0;
	...
	while (ALWAYS) //   read as many tokens as needed to complete the definition (must be within same file)
	{
		...
		if (undeclared)
		{
			...
			if (!more)
			{
				if (D->internalBits & CONCEPT && D->internalBits & (BUILD0|BUILD1))
					BADSCRIPT("CONCEPT-3 Concept/topic already defined %s",conceptName)
			}
			AddInternalFlag(D,build|CONCEPT);
		}
		...
		switch(*word) //   THE MEAT OF CONCEPT DEFINITIONS
		{
			...
			case ')': case ']':		// end keyword list
				...
				break;
			default: 
				 ptr = ReadKeyword(word,ptr,notted,quoted,concept,type,ignoreSpell,build);
		}
	}
...
static void WriteDictionaryChange(FILE* dictout, uint64 build)
{
	...
	else in = FopenReadWritten("TMP/prebuild1.bin");
	if (!in) ReportBug("4se file")
	for (WORDP D = dictionaryBase+1; D < dictionaryFree; ++D) 
	{
		...
		if (D->internalBits & QUERY_KIND && D->internalBits & build) 
		{
		...
		else if (D->internalBits & SUBSTITUTE_RECIPIENT) continue; // ignore pattern words, etc EXCEPT when field of a fact
		...
		if (!D->properties && !D->systemFlags && D->internalBits & CONCEPT && D <= dictionaryPreBuild0 ) {;}  // preexisting word a concept
...
static void WriteExtendedFacts(FILE* factout,FILE* dictout,uint64 build)
{
	...
	shared = false;
	char* ptr = WriteVariables(buffer,false);
	shared = oldshared;
...
static void ClearTopicConcept(WORDP D, uint64 build)
{
	...
	if ((D->internalBits & (TOPIC | CONCEPT)) & k)   RemoveInternalFlag(D,CONCEPT|BUILD0|BUILD1|TOPIC);
}
...
static unsigned int DumpWarnings(unsigned int& substitutes,unsigned int &cases)
{
...
void ReadTopicFiles(char* name,uint64 build,int spell)
{
	...
	{
		char file[MAX_WORD_SIZE];
		sprintf(file,"RAWDATA/PRIVATE/%s",name); // 2nd default is rawdata / private
		in = FopenReadNormal(file);
		if (!in)
		{
			sprintf(file,"../%s",name); // 3rd default is just above chatscript folder
			in = FopenReadNormal(file);
			if (!in)
			{
				printf("%s not found\r\n",name);
	...
	*newBuffer = 0;
	...
	out = FopenUTF8WriteAppend(build == BUILD0 ? "TOPIC/script0.txt" : "TOPIC/script1.txt","rb+");
	if (out)
	{
	...
	if (hasPlans)
	{
		out = FopenUTF8WriteAppend(build == BUILD0 ? "TOPIC/plans0.txt" : "TOPIC/plans1.txt","rb+");
		if (out)
		{
			char word[MAX_WORD_SIZE];
			fseek(out,0,SEEK_SET);
	...
	else if (hasWarnings) 
	{
		...
		unsigned int cases;
		unsigned int count = DumpWarnings(substitutes,cases);
		if (missingFiles) Log(STDUSERLOG,"%d topic files were missing.\r\n",missingFiles);
		Log(STDUSERLOG,"%d spelling warnings, %d case warnings, %d substitution warnings,  and %d more serious warnings\r\n    ",count,cases,substitutes,hasWarnings-count-substitutes-cases);
	}
	else 
	{
		if (missingFiles) Log(STDUSERLOG,"%d topic files were missing.\r\n",missingFiles);
