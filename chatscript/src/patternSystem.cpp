...
bool Match(char* ptr, unsigned int depth, int startposition, char kind, bool wildstart,unsigned int& gap,unsigned int& wildcardSelector,unsigned int &returnstart, unsigned int& returnend,bool &uppercasem,bool reverse)
	...
	while (ALWAYS) //   we have a list of things, either () or { } or [ ].  We check each item until one fails in () or one succeeds in  [ ] or { }
    {
        ...
        switch(c) 
        {
			...
			case '_': //     memorization coming - there can be up-to-two memorizations in progress: _* and _xxx  OR  simply names a memorized value like _8
				...
				uppercasematch = false;
				wildcardSelector |= (word[1] == '*' && !IsDigit(word[2]) && word[2] != '-' && word[2] != '*') ? WILDGAP :  WILDSPECIFIC; // select duration of memorization - **word is a specific word
				ptr -= strlen(word); 
          ...
          case 0: // end of data (argument or function - never a real rule)
            ...
            case '(': case '[':  case '{': // nested condition (required or optional) (= consecutive  [ = choice   { = optional
                hold = wildcardIndex;
				{
					...
					matched = Match(ptr,depth+1,positionEnd,*word, positionStart == INFINITE_MATCH,gap,wildcardSelector,returnStart,returnEnd,uppercasemat,reverse); //   subsection ok - it is allowed to set position vars, if ! get used, they dont matter because we fail
            ...
            case '\\': //   escape to allow [ ] () < > ' {  } ! as words and 's possessive And anything else for that matter
				if (word[1] == '!' ) matched =  (wordCount && (tokenFlags & EXCLAMATIONMARK)); //   exclamatory sentence
		...
		if (reverse)
		{
			...
			else if (matched && oldp != INFINITE_MATCH && positionEnd < (olde - 1 ) && positionStart != INFINITE_MATCH )  // failed to match position advance
			{
				...
				if (oldp && unmarked[ignored]) while (--ignored > positionEnd && unmarked[ignored]); // dont have to account for these
