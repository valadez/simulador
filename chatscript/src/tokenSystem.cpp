...
static char* FindWordEnd(char* ptr,char* priorToken,char** words,int &count,bool nomodify)
{
	...
	if (kind & QUOTERS) // quoted strings 
	{
		...
		else if ((c == '\'' || c == '"') && tokenControl & TOKEN_AS_IS) {;} // for penntag dont touch it - for 've  leave it alone also leave '82 alone
		else
		{
...
{	// all is true if to pay no attention to end of sentence -- eg for a quoted string
	...
	*priorToken = 0;
	...
	{
		if (!*ptr) break; 
		if (!(tokenControl & TOKEN_AS_IS)) while (*ptr == ptr[1] && !IsAlphaUTF8OrDigit(*ptr)  && *ptr != '-' && *ptr != '.') ++ptr; // ignore repeated non-alpha non-digit characters -   - but NOT -- and not ...
		...
		if (!token) token = words[count] = "a";
		else if (len == 1 && startc == 'i') *token = 'I'; // force uppser case on I
...
{ // n is words merged into "word"
    ...
    if (!word || word->systemFlags & CONDITIONAL_IDIOM) return 0; // can not or dare not
	...
	if (word->internalBits & HAS_SUBSTITUTE)
	{
		...
		return 0; // disallowed substitution
	}
    ...
    if (word->properties & (NOUN|ADJECTIVE|ADVERB|CONJUNCTION_SUBORDINATE)) return word; // merge dictionary found normal word
	return 0;
}
