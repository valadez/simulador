...
{ // this is not allowed to write properties/systemflags/internalbits if the word is preexisting
	...
	if (ZZ && ZZ->properties & (DETERMINER|PREPOSITION|PRONOUN_POSSESSIVE|PRONOUN_BITS|AUX_VERB) && !IsNumber(original)) // prep and determiner are ALWAYS considered lowercase for parsing (which happens later than proper name extraction)
	{
		entry =  canonical = ZZ;
	...
	else if (start != at && tokenControl & STRICT_CASING) {;} // believe all upper case items not at sentence start

	else if (!stricmp(original,"the") || !stricmp(original,"a")) // force lower case on these determiners regardless
	{
		entry =  canonical = ZZ;
		wordStarts[at] = entry->word;
		original = entry->word;
	...
	// numeric words
	if ((IsDigit(*original) || IsDigit(original[1]) || *original == '\'') && kind)
	{
	...
	if (!(properties & VERB_BITS) && !IsUpperCase(*original)) // could it be a verb we dont know directly (even if we know the word)
	{
		...
		if (verb)  // inifinitive will be different from original or we would already have found the word
		{
			...
			if (verbFormat & (VERB_PAST|VERB_PAST_PARTICIPLE)) // possible shared form with participle
			{
				...
				if (pastparticiple && !strcmp(pastparticiple,original)) properties |= VERB_PAST_PARTICIPLE | NOUN_ADJECTIVE;
			}
			entry = StoreWord(original,properties);
	...
	if (!(properties & ADJECTIVE_BITS) && !IsUpperCase(*original) && len > 3) // could it be comparative adjective we werent recognizing even if we know the word
	{
		if (original[len-1] == 'r' && original[len-2] == 'e' && !canonical) // if canonical its like "slaver" and should never be reduced
		{
			char* adjective = GetAdjectiveBase(original,true);
			if (adjective && strcmp(adjective,original)) 
			{
				WORDP D = StoreWord(original,ADJECTIVE|ADJECTIVE_NORMAL|adjectiveFormat);
				if (!entry) entry = D;
		...
		else if (original[len-1] == 't' && original[len-2] == 's'  && original[len-3] == 'e' && !canonical)
		{
			char* adjective = GetAdjectiveBase(original,true);
			if (adjective && strcmp(adjective,original)) 
			{
	...
	if (!(properties & ADVERB) && !IsUpperCase(*original) && len > 3) // could it be comparative adverb even if we know the word
	{
		if (original[len-1] == 'r' && original[len-2] == 'e'  && !canonical)
		{
	...
	if (!preknown) // if we didnt know the original word, then even if we've found noun/verb forms of it, we need to test other options
	{
		...
		{
			char* adjective = GetAdjectiveBase(original,true); 
			if (len > 4 && !adjective && hyphen && !strcmp(original+len-3,"ing")) // some kind of verb ing formation which can be adjective particple
			{
				*hyphen = 0;
				char word[MAX_WORD_SIZE];
				WORDP X = FindWord(original,0,LOWERCASE_LOOKUP);
		...
		if (hyphen  && !IsUpperCase(*original)) // unknown hypenated words even when going to base
		{
			WORDP X;
	...
	else 
	{
		...
		WORDP X = (at == start) ? FindWord(wordStarts[at],0,UPPERCASE_LOOKUP) : NULL;
...
char* English_GetInfinitive(char* word, bool nonew)
{
    ...
    if (last == 's' && len > 3 && prior != 'u' && prior != '\'') // but should not be "us" ending (adjectives)
    {
 		verbFormat = VERB_PRESENT_3PS;
		bool es = false;
		if (prior == 'e' && word[len-3] == 'h' && (word[len-4] == 's' || word[len-3] == 'c'))  es = true;
