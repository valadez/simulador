...
void InitChatbot(char* name)
{
	...
	char init[10];
	*init = 0;
	PerformChat(loginID,"",init,NULL,mainOutputBuffer);     // start up chat with a judge
}
...
VOID CALLBACK TimeCheck( HWND hwnd, UINT uMsg,UINT_PTR idEvent,DWORD dwTime) // every 100 ms
{
	...
	*oob = 0;
	...
	if (trigger || *oob)
	{
		if (*oob) strcpy(mainInputBuffer,oob); // priority is to alarm callback. others will be only if no user input anyway
		else  
		{
			strcpy(mainInputBuffer,inputMessage);
		...
		PerformChat(loginID,computerID,mainInputBuffer,NULL,mainOutputBuffer);
		strcpy(response,mainOutputBuffer);
		callBackDelay = 0; // now turned off after an output
		ProcessOOB(mainOutputBuffer); // process relevant out of band messaging and remove

		char word[MAX_WORD_SIZE];
		char* p = SkipWhitespace(mainOutputBuffer);
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_PAINT:
		...
		else  TextOut(hdc, 20, 100, mainInputBuffer, _tcslen(mainInputBuffer));  // last input seen
